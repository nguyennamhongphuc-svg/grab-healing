...
# HÀM GIÚP TÌM ĐỊA CHỈ THÔNG MINH (TỰ ĐỘNG THỬ LẠI & CHỐNG NGHẼN)
def smart_geocode(address):
    # Mỗi lần gọi sẽ dùng một tên app ngẫu nhiên để tránh bị máy chủ chặn
    user_agents = [f"healing_app_user_{random.randint(1000, 9999)}" for _ in range(10)]
    
    for i in range(3): # Thử tối đa 3 lần nếu gặp lỗi mạng
        try:
            ...
            if result:
                return result
            time.sleep(1) # Nghỉ 1 giây giữa các lần thử
        except:
            time.sleep(1.5)
    return None

# ==========================================
# CSS (PHONG CÁCH HỒNG TRẮNG CHỮA LÀNH)
# ==========================================
st.markdown("""
<style>
...
# SỬA: Sử dụng placeholder thay vì nhập sẵn địa chỉ
in_don = st.text_input("📍 Điểm đón hiện tại...", placeholder="Ví dụ: Landmark 81, TP. HCM")
in_den = st.text_input("📍 Nơi muốn đến...", placeholder="Ví dụ: Vũng Tàu")

dr_mood = st.selectbox("🧠 Tâm trạng hiện tại", ['Cần tâm sự', 'Cần yên tĩnh', 'Đang rất vui', 'Căng thẳng'])
dr_gender = st.selectbox("👤 Ưu tiên tài xế", ['Nam', 'Nữ', 'Bất kỳ'])
dr_age = st.selectbox("🎂 Độ tuổi mong muốn", ['18-25', '26-35', '36-50'])
...
# ==========================================
# LOGIC XỬ LÝ VÀ HIỂN THỊ HÀNH TRÌNH
# ==========================================
if st.session_state.step > 0:
    # Sử dụng hàm smart_geocode đã tối ưu
    ...
    if loc_don and loc_den:
        ...
        # Giả lập vị trí các tài xế xung quanh
        random.seed(42)
        ...
        # Cấu hình thông tin tài xế theo tâm trạng
        mood_configs = {
            'Cần tâm sự': ('Trần Minh Anh', 'VinFast VF8', 'Thấu cảm, ấm áp', 'Tài xế lắng nghe như một người bạn thân.'),
            'Cần yên tĩnh': ('Lê Quốc Bảo', 'Toyota Camry', 'Lịch sự, điềm đạm', 'Không gian yên tĩnh để bạn nghỉ ngơi.'),
            'Đang rất vui': ('Nguyễn Hoàng Nam', 'Mazda 6', 'Năng động, hóm hỉnh', 'Chuyến xe ngập tràn năng lượng tích cực!'),
            'Căng thẳng': ('Đặng Mỹ Hạnh', 'Honda Accord', 'Nhẹ nhàng, tinh tế', 'Âm nhạc du dương giúp xua tan áp lực.')
        }
        ...
        def full_info_card(status_title):
            return f"""<div class='driver-info-card'>
                ...
                <div style='margin-top:15px; font-size:16px; color:#1f2937; line-height: 1.7;'>
                    ...
                    🛣️ Kinh nghiệm: <b>{trips} chuyến đi</b><br>
                    💬 Nhận xét: <span style='color:#4b5563;'>"{feedback}"</span>
                </div>
                <div class='price-tag'>{(15000 + dist_final*12000):,.0f} VNĐ</div>
            </div>"""

        # BƯỚC 1: ĐANG TÌM TÀI XẾ
        if st.session_state.step == 1:
            st.markdown("<p style='text-align:center;'>🔍 <b>Đang quét tần số kết nối tài xế phù hợp...</b></p>", unsafe_allow_html=True)
        ...
        # BƯỚC 3: TRÊN HÀNH TRÌNH CHỮA LÀNH
        ...
        # Xử lý lỗi nếu không tìm thấy địa chỉ hoặc server bận
        st.error("⚠️ Dịch vụ định vị đang bận hoặc địa chỉ không rõ ràng. Bạn vui lòng đợi 3 giây rồi bấm lại nhé!")
        st.session_state.step = 0
