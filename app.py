...
# SỬA LỖI TẠI ĐÂY: Thêm user_agent riêng biệt và tăng timeout lên 10 giây
geolocator = Nominatim(user_agent="grab_healing_phuc_unique_v1", timeout=10)
...
if st.session_state.step > 0:
    # SỬA LỖI TẠI ĐÂY: Dùng try-except để bắt lỗi Geocoder khi server bận
    try:
        ...
        if loc_don and loc_den:
            ...
            st.error("Không tìm thấy địa chỉ bạn nhập. Vui lòng kiểm tra lại!")
            st.session_state.step = 0

    except Exception:
        st.warning("⚠️ Dịch vụ định vị đang bận. Bạn vui lòng đợi 2-3 giây rồi bấm lại nhé!")
        st.session_state.step = 0
